...
export const downloadSlackTeamAccessLogs = (token, selectedFile) => async (dispatch) => {
    dispatch(setErrorSlackTeamAccessLogs(null));
    dispatch(setLoadingSlackTeamAccessLogs(true));
    try {
        const accessLogsData = await apiSlackTeamService.getSlackTeamAccessLogsData(token, selectedFile);
        dispatch(setSlackTeamAccessLogs(accessLogsData));
    } catch (error) {
        dispatch(setErrorSlackTeamAccessLogs(error?.response?.data?.message || error?.message || error?.toString()));
